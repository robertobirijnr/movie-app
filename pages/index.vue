<template>
  <div class="home">
    ...
    <div class="container search">
      ...
      <button  @click="clearchSearch" v-show="searchInput !==''" class="button">Clear Search</button>
    </div>
...
export default {
  ...
  methods:{
    async getMovies(){
      const data = axios.get(`${process.env.BASE_URL}/movie/now_playing?api_key=${process.env.API}&language=en-US&page=1`)
    ...
    async searchMovies(){
      const data = axios.get(`${process.env.BASE_URL}/search/movie?api_key=${process.env.API}&language=en-US&page=1&query=${this.searchInput}`)
      const result = await data
      result.data.results.forEach((movie)=>{
